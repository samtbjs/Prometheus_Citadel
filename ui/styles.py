...
/* =======================================================================
   ATMOSPHERE PASS -- living background + entrance motion for the
   Streamlit chrome around the existing 3D scenes (those already animate
   via GSAP/Three.js in ui/motion.py + scenes/*.html and are untouched).
   Everything here is additive: new selectors/pseudo-elements/keyframes
   only, GPU-cheap (opacity/transform/background-position), and fully
   disabled under prefers-reduced-motion to match the reduced-motion
   contract the 3D scenes already honor (PM_REDUCED_MOTION in motion.py).
   ======================================================================= */
...
.stApp::before {
    ...
    background-image:
        ...
        radial-gradient(1px 1px at 65% 55%, rgba(255,255,255,0.7), transparent 60%),
        radial-gradient(1.5px 1.5px at 35% 80%, rgba(255,255,255,0.6), transparent 60%),
        radial-gradient(1px 1px at 85% 15%, rgba(255,255,255,0.8), transparent 60%),
        radial-gradient(1px 1px at 50% 40%, rgba(255,255,255,0.5), transparent 60%),
        radial-gradient(1.5px 1.5px at 90% 75%, rgba(255,255,255,0.7), transparent 60%);
    background-size: 600px 600px;
    background-repeat: repeat;
    animation: pl-star-drift 90s linear infinite, pl-twinkle 6s ease-in-out infinite;
    opacity: 0.5;
}
@keyframes pl-star-drift {
...
/* ---- Occasional holographic scan-line sweep ---------------------------- */
.stApp::after {
    content: "";
    position: fixed;
    inset: 0;
    ...
    linear-gradient(to bottom, transparent 0%, var(--accent-glow) 50%, transparent 100%);
    background-size: 100% 100%, 100% 220px;
    background-repeat: repeat, no-repeat;
    background-position: 0 0, 0 -220px;
    opacity: 0.06;
    mix-blend-mode: screen;
    animation: pl-scan-sweep 9s ease-in-out infinite;
}
@keyframes pl-scan-sweep {
    0%   { background-position: 0 0, 0 -220px; }
    45%  { background-position: 0 0, 0 100%; }
...
/* ---- Page entrance: nothing pops in, everything rises + fades --------- */
@keyframes pl-rise-in {
    from { opacity: 0; transform: translateY(16px); }
    to   { opacity: 1; transform: translateY(0); }
}
h1 { animation: pl-rise-in 0.7s ease-out both; }
h2 { animation: pl-rise-in 0.7s ease-out 0.06s both; }
h3 { animation: pl-rise-in 0.7s ease-out 0.1s both; }
[data-testid="stMetric"] { animation: pl-rise-in 0.7s ease-out 0.16s both; }
[data-testid="stAlert"] { animation: pl-rise-in 0.5s ease-out both; }
[data-testid="stVerticalBlockBorderWrapper"] { animation: pl-rise-in 0.7s ease-out 0.12s both; }
.stButton { animation: pl-rise-in 0.6s ease-out 0.2s both; }

/* Stagger sibling columns (mission cards / chapter stations) so they
   settle in one after another instead of all at once. */
[data-testid="column"]:nth-of-type(1) { animation-delay: 0.05s; }
[data-testid="column"]:nth-of-type(2) { animation-delay: 0.14s; }
[data-testid="column"]:nth-of-type(3) { animation-delay: 0.23s; }
[data-testid="column"]:nth-of-type(4) { animation-delay: 0.32s; }
[data-testid="column"]:nth-of-type(5) { animation-delay: 0.41s; }
[data-testid="column"] { animation: pl-rise-in 0.6s ease-out both; animation-delay: inherit; }

/* ---- Idle "breathing" glow so key panels never look frozen ------------ */
@keyframes pl-breathe {
    0%, 100% { box-shadow: 0 0 14px var(--accent-glow); }
    50%      { box-shadow: 0 0 22px var(--accent-glow); }
}
button[kind="primary"] { animation: pl-breathe 3.2s ease-in-out infinite; }
[data-testid="stMetricValue"] { animation: pl-twinkle 4s ease-in-out infinite; }

/* ---- Respect reduced motion: same contract as the 3D scenes ----------- */
@media (prefers-reduced-motion: reduce) {
    .stApp::before, .stApp::after,
    h1, h2, h3, [data-testid="stMetric"], [data-testid="stAlert"],
    [data-testid="stVerticalBlockBorderWrapper"], .stButton, [data-testid="column"],
    button[kind="primary"], [data-testid="stMetricValue"] {
